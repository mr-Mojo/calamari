...
class AbbyyDatasetGenerator(DatasetGenerator):
    ...
    def _load_sample(self, sample, text_only):
        ...
        if self.mode == DataSetMode.TRAIN or self.mode == DataSetMode.PREDICT or self.mode == DatasetMode.PREDICT_AND_EVAL:
        ...
        for p, page in enumerate(book.pages):
            for l, line in enumerate(page.getLines()):
                for f, fo in enumerate(line.formats):
                    text = None
                    cut_img = None
                    if self.mode == DataSetMode.EVAL or self.mode == DataSetMode.TRAIN or self.mode == DataSetMode.PREDICT_AND_EVAL:
                        text = fo.text
