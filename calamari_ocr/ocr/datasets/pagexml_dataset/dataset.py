...
class PageXMLDatasetGenerator(DatasetGenerator):
    ...
    def _load_sample(self, sample, text_only):
        ...
        for sample in loader.load(image_path, xml_path):
            ...
            if not text_only and (self.mode == DataSetMode.PREDICT or self.mode == DataSetMode.TRAIN or self.mode == DataSetMode.PREDICT_AND_EVAL):
                ly, lx = img.shape
...
class PageXMLDatasetLoader:
    ...
    def load(self, img, xml, skip_commented=True):
        ...
        if self.mode == DataSetMode.TRAIN or self.mode == DataSetMode.EVAL or self.mode == DataSetMode.PREDICT_AND_EVAL:
